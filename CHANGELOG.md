...
- **Expérience `share-meteo@2`** (addendum de l'[ADR-0028](docs/adr/0028-prevision-part-renouvelable-eligibilite.md)) —
  ...
  sur `share-clim@1`** hors couverture météo. La sous-commande `backtest-share`
  exécute désormais une **comparaison à trois** (météo vs climatologie vs
  persistance, mêmes origines/cibles) quand un historique météo est disponible.
  GO formel du gate sur les deux fenêtres de l'ADR-0028 (bat `share-clim@1` en
  RMSE global, 0 faux verdict), gain concentré à h+1/h+6 (−7,7 %/−6,6 % à h+1 ;
  parité par construction au-delà de la couverture d'archive 24 h). **Non
  servi** : `share-clim@1` reste le modèle en production, la promotion est une
  décision produit documentée dans l'addendum.
