...
- ADR-0010 — `acv-ademe` **consumption-based** (**Accepté, engagé**) : imports valorisés à l'intensité du pays d'origine (ENTSO-E via `adapter-entsoe`), `MethodologyCalculator` (trait domaine pur), endpoints `/v1/methodologies` & `/v1/factors`. **Fait évoluer** l'ADR-0008 (livré en `@1` production-based). **Tranche A livrée** : domaine pur (`MethodologyCalculator`/`RteDirect`/`AcvAdemeProduction`/`AcvAdemeConsumption`, `CrossBorderFlows`, calcul `acv-ademe@2` + pertes T&D) + endpoints de vérifiabilité. **Tranche B 1/2 livrée** : port `CrossBorderSource` + `CrossBorderSnapshot` + crate `adapter-entsoe` (flux A11 + intensité voisine via génération A75 ; chemins XML à valider live). **B 2/2** (à venir) = store Postgres + ingestion poller + service de `@2`.
...
- [ ] Phase 4 — **enrichissement & usage** (ADR proposés 0010, 0012-0014) :
  - [~] `acv-ademe` **consumption-based** + `adapter-entsoe` + `/v1/factors` (ADR-0010) :
    ...
    - [~] **tranche B** — ENTSO-E :
      ...
      - [ ] **B 2/2** : `CrossBorderRepository` Postgres (migration + store) + ingestion poller + service effectif de `acv-ademe@2` à la lecture (`AcvAdemeConsumption` sur mix FR + snapshot). `@2` reste `planned` tant que non câblé.
