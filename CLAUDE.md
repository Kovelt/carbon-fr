...
- ADR-0025 — **extension hydrogène carbon-aware** (**Accepté, non implémenté**) : sert l'opportunité H₂ comme **extension de la couche carbon-aware** (ADR-0014), **pas** un produit sœur. **Couche A** = overlay « électrolyseur » (helper/endpoint mince) évaluant l'éligibilité **au niveau réseau** par créneau, sous deux cadres explicitement étiquetés et neutres — `rfnbo` (renouvelable, Règl. délégués UE 2023/1184-1185) et `low-carbon` (bas-carbone inclusif nucléaire/CCS, acte délégué 8/7/2025) — au-dessus des données régionales horodatées et de `/greenest-window`. **Hors périmètre** : gCO₂eq/kgH₂ et certification (donnée au niveau site absente). ⚠️ La corrélation **géographique** s'évalue à la **zone de dépôt nationale** (FR = 1 zone), pas aux 12 sous-régions. Seuils **paramétrables/versionnés** (fenêtre de corrélation temporelle, bascule horaire `2030-01-01`, exemption Article 4 ≥ 90 % renouvelables, surplus prix < 20 €/MWh → port prix day-ahead). Méthodologie fine = **ADR-0026** (pressenti). Brief d'implémentation : [`docs/brief-claude-code-couche-A-electrolyseur.md`](docs/brief-claude-code-couche-A-electrolyseur.md). **Couches B-light** (carte infra × carbone live) et **B-full** (observatoire H₂) documentées en réserve.
