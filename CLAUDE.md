...
- ADR-0026 — **méthodologie des overlays d'éligibilité** (**Accepté, implémenté**) : crate domaine pur `carbonfr-eligibility` (rulesets versionnés `rfnbo:2023-1184` / `low-carbon:2025-2359` servis + `rfnbo:2026-revision` planifié non servi). `rfnbo` = **disjonction** des exceptions réseau (part renouvelable instantanée ≥ 0,90 **OU** prix day-ahead ≤ 20 €/MWh) ; `low-carbon` = **condition nécessaire** intensité ≤ seuil **dérivé** `round(3384/53) ≈ 64 gCO₂eq/kWh` (proxy `indicative`, non réglementaire — l'acte 2025/2359 ne fixe qu'un seuil produit). Exposé via `GET /v1/intensity/greenest-window?eligibility=rfnbo|low-carbon` (param orthogonal à `methodology`, réponse additive) + catalogue `GET /v1/eligibility/rulesets`. **Mono-forecast** (un seul `forecast()` partagé fenêtre/éligibilité), estimateur propagé, `low-carbon` indéterminé si le seuil ∈ [lower, upper] (ADR-0011). **Piège 1** : `bidding_zone` toujours `FR`. **Piège 2** : prix indéterminé au-delà du day-ahead (filtre ≤ 1 h), jamais extrapolé. Part renouvelable future = nowcast-only (ancre `rte-direct`) → `MixForecast` réservé. `core` intact (orchestration côté adapter HTTP). Branche EUA du surplus & additionnalité PPA **hors périmètre**, documentées dans le `disclaimer`. Brief d'implémentation : [`docs/brief-claude-code-couche-A-electrolyseur.md`](docs/brief-claude-code-couche-A-electrolyseur.md) ; plan : [`docs/plan-couche-A-electrolyseur.md`](docs/plan-couche-A-electrolyseur.md). **Addendum de vérification sources primaires (2026-07-03)** dans l'ADR : pas de seuil électrique dans l'annexe 2025/2359 (proxy `indicative` confirmé ; 4 méthodes de comptabilisation réseau dont 2 horaires), révision RFNBO **non adoptée** (`rfnbo:2026-revision` reste `planned`). **GATE de neutralité franchi le 2026-07-03 (GREEN)** par évaluation adversariale multi-agents (revue datée [`docs/adr/0026-revue-neutralite.md`](docs/adr/0026-revue-neutralite.md), re-jouable) : 3 correctifs — `basis: user-override` sur pilier surchargé (flags granulaires + `basis_for`), doc `score` interne-au-cadre (OpenAPI + SDK), `legal_basis` low-carbon (renvoi 2023/1185 + considérant non contraignant). **Suite séquencée par déclencheurs** : [`docs/roadmap-hydrogene.md`](docs/roadmap-hydrogene.md) (H3 = activer `rfnbo:2026-revision` sur texte adopté ; signaux de veille listés). **H6 livré (ADR-0029, 2026-07-03)** : carte `GET /hydrogene` (hors `/v1`, comme `/docs`) — page auto-contenue (zéro CDN/tuile/lib, SVG maison, palettes validées) : 233 électrolyseurs UE (EHO © Clean Hydrogen JU, semestriel, filtre Water electrolysis) × choropleth régional `acv-ademe` live + fenêtres rfnbo/low-carbon + SSE ; assets embarqués `include_str!` dans `crates/adapter-http/assets/hydrogene/` (procédure de rafraîchissement dans l'ADR) ; fond IGN Licence Ouverte + Natural Earth (GISCO écarté — clause EuroGeographics ; Vig'Hy écarté — pas de licence ; ADEME `hyd01-sites` = v2 après confirmation écrite `cdo@ademe.fr`). ⚠️ Jamais d'éligibilité **par site** (garde-fou testé). **H4 livré (ADR-0028, 2026-07-03)** : `share-clim@1` = part renouvelable **prévue** du pilier rfnbo (climatologie horaire-de-semaine + intervalle calibré par horizon, verdict par règle d'intervalle D17, provenance `observed|forecast` + `reason` d'indétermination servis, horizon calibré 72 h, `CARBONFR_SHARE_CALIBRATE_WEEKS` 0=off) — gate `backtest-share` (bat la persistance, 0 faux verdict/450) + GATE de neutralité re-joué GREEN (revue §6, correctifs F1/F6/F12 = parité de provenance entre piliers). ⚠️ distinct du `MixForecaster` GBDT (ADR-0013).
