...
- ADR-0014 — **usage** (**Accepté, engagé**) : primitives carbon-aware (créneau sous échéance, lowest-k, seuil, annotation d'économie) + livraison live **SSE** (`/v1/intensity/stream`, `LISTEN`/`NOTIFY`) ; webhooks reportés (gated sur le tier hébergé). **Tranche A livrée** : primitives pures + `CarbonAwareScheduler` + endpoints `/v1/schedule`, `/v1/schedule/slots`, `/v1/intensity/below`. **Tranche B** (à venir) = SSE.
...
- [ ] Phase 4 — **enrichissement & usage** (ADR proposés 0010, 0012-0014) :
  - [x] `acv-ademe` **consumption-based** + `adapter-entsoe` + `/v1/factors` (ADR-0010) :
    ...
    - [x] **tranche B** — ENTSO-E :
      ...
      - [x] **B 2/2** : `CrossBorderRepository` Postgres (migration `0007`, table `cross_border_flow` `(at, neighbor)`, `upsert_flows`/`flows_at`, testé sur Postgres réel) + ingestion poller (si `CARBONFR_ENTSOE_TOKEN`, source optionnelle) + service `acv-ademe@2` à la lecture (cas d'usage `GetConsumptionIntensity`, calcul **à la lecture** sans stockage `@2`) via `GET /v1/intensity/now?methodology=acv-ademe&version=2` (national). `@2` = `served` dans `/v1/methodologies`. **Reste** : valider chemins XML ENTSO-E live, sourcer le facteur T&D, `@2` sur historique/stats (rollups de variante).
  ...
  - [~] **usage** : primitives de scheduling carbon-aware + streaming **SSE** (ADR-0014) ; webhooks reportés (tier hébergé) :
    ...
    - [ ] **tranche B** — livraison live **SSE** (`GET /v1/intensity/stream`, Postgres `LISTEN`/`NOTIFY`). Webhooks reportés (gated ADR-0015).
