...
- [ ] Phase 4 — **enrichissement & usage** (ADR proposés 0010, 0012-0014) :
  - [~] `acv-ademe` **consumption-based** + `adapter-entsoe` + `/v1/factors` (ADR-0010) :
    ...
    - [ ] **tranche B** — adapter `adapter-entsoe` + port `CrossBorderSource` + store du contexte d'import + ingestion poller + service effectif de `acv-ademe@2` (statut `planned` dans `/v1/methodologies` tant que la source n'est pas branchée).
