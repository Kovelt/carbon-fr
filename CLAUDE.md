...
- **Suite du projet** : [`docs/plan-iterations.md`](docs/plan-iterations.md) — itérations I0→I7 (file de PR, donnée & exploitation, crates.io : ADR-0030 → préparation → publication, SDK Rust, montées majeures), échéances datées et chantiers en attente de déclencheur. Le tenir à jour (cases cochées, date en tête) à chaque PR qui avance un item.
...
- **Déploiement prod** : procédure générale dans `deploy/README.md` §2 (compose + Traefik, image GHCR épinglée ; le fichier compose de prod vit sur le VPS, **pas dans le dépôt** — `deploy/Caddyfile` et `deploy/carbonfr.service` ne sont que des exemples d'auto-hébergement). ⚠️ L'accès au VPS n'est **jamais** dans le dépôt (public) : il vit dans la **mémoire de session locale** (`prod-vps-kovelt-acces`) — la lire AVANT toute connexion, ne jamais deviner/sonder des identifiants SSH.
...
- ENTSO-E : chemins XML validés **live** (A11/A75 le 2026-06-16, A44 le 2026-06-20) et **rejoués le 2026-09-23** après le correctif du parseur A03 (0.7.0) et `quick-xml` 0.42 — à rejouer après toute montée de `quick-xml`/`reqwest` ou tout changement du parseur (`cargo test -p carbonfr-adapter-entsoe --test live -- --ignored`, `CARBONFR_ENTSOE_TOKEN` : celui de prod, dans le `.env` du VPS, à ne jamais afficher) et dater le rejeu dans `crates/adapter-entsoe/src/lib.rs`. (`TD_LOSS_FACTOR_V1 = 0,072` est, lui, sourcé : ADR-0010 §3 + `factors.rs`, bilans RTE/Enedis.)
...
- **Backfill** : port `Eco2mixArchive` (export de masse, dataset `eco2mix-national-cons-def` — ou `eco2mix-national-tr` avec `CARBONFR_BACKFILL_SOURCE=realtime` pour un trou récent pas encore consolidé, que RTE publie avec ~3 mois de retard), cas d'usage `BackfillHistory` qui **découpe en tranches** (une tranche = un export, pas l'API paginée — ADR-0003). Jamais de backfill via `range()` (plafonné).
...
- **Partitionnement mensuel** (ADR-0004) : toujours reporté (table simple, cf. commentaire de la migration `0001`) ; l'index BRIN, lui, est livré (migration `0012`). À reconsidérer maintenant que l'historique complet est ingéré.
- **OpenAPI code-first** (`utoipa`) : `ToSchema` sur les **DTO de l'adapter HTTP** uniquement (jamais le `core`), `#[utoipa::path]` sur les handlers (fonctionne malgré la généricité), `ApiDoc` dans `carbonfr_openapi.rs` → `/v1/openapi.json` + Swagger UI `/docs`. Collection **Bruno** dans `bruno/`.
