...
│   ├── adapter-odre/         # impl Eco2mixSource (HTTP → ODRÉ)        [à venir]
│   ├── adapter-postgres/     # impl IntensityRepository (sqlx/Postgres) [à venir]
│   └── adapter-http/         # API axum (adapter entrant)              [à venir]
└── bin/
    └── server/               # composition root : câble les adapters    [à venir]
```
...
cargo fmt --all
# Adapters (à venir) :
# sqlx migrate run          # migrations Postgres
# cargo run -p server       # lance l'API
```
...
- [ ] Phase 1 — socle `core` + adapters ODRÉ/Postgres/HTTP.
