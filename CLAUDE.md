...
- [x] Phase 5 — **enrichissement, déploiement & SDK** :
  ...
  - [x] **couche comparative LCOE** (ADR-0024) : `GET /v1/cost-reference` — fourchette d'estimations par filière (`cost.rs`), nucléaire scindé existant/nouveau, périmètre `plateau` explicité, note neutre, jamais d'écart calculé. **GATE de neutralité franchi** par évaluation adversariale multi-agents (pro/anti-nucléaire + audits), revue datée `docs/adr/0024-revue-neutralite.md`. **Licences confirmées** (recherche 2026-06-20 : Licence Ouverte ADEME, CRPA CdC, faits non protégés RTE) — réutilisation chiffres-faits sous conditions. Reste (gouvernance) : multi-source par filière ; confirmation écrite RTE avant palier payant sur sa donnée.
