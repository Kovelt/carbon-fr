...
- ⚠️ ENTSO-E : chemins XML validés **live** (A11/A75 le 2026-06-16, A44 le 2026-06-20) mais **aucun rejeu documenté depuis le correctif du parseur A03 de v0.7.0** (signe des flux) ni depuis les montées de `quick-xml` — re-valider avant de s'y fier (`cargo test -p carbonfr-adapter-entsoe --test live -- --ignored`, `CARBONFR_ENTSOE_TOKEN`) et dater le rejeu dans `crates/adapter-entsoe/src/lib.rs`. (`TD_LOSS_FACTOR_V1 = 0,072` est, lui, sourcé : ADR-0010 §3 + `factors.rs`, bilans RTE/Enedis.)
