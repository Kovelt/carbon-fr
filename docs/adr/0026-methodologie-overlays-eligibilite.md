...
## Addendum — alignement sur les méthodes horaires de l'annexe du 2025/2359 (O1, 2026-07-04)

Clarification **documentaire** (chantier O1 de la [roadmap hydrogène](../roadmap-hydrogene.md)) : positionner la donnée carbon-fr par rapport aux **quatre méthodes** de comptabilisation de l'électricité réseau définies par l'annexe du Règl. (UE) 2025/2359 (partie A, point 6 — cf. addendum du 2026-07-03). Aucun nouveau champ d'API, aucun nouveau ruleset, aucun seuil modifié.

**Principe.** carbon-fr produit des signaux de la même *famille* que les méthodes horaires (b) et (d) — intensité horaire à l'échelle de la bidding zone, technologie marginale — mais **n'implémente littéralement aucune des quatre méthodes** : (b) et (d) exigent une donnée **publiée par le GRT** (RTE ne publie ni prévision d'intensité day-ahead de bidding zone au sens de (b), ni marginale horaire au sens de (d)), (a) repose sur la table officielle (Table 5) et (c) sur un comptage d'heures pleine charge non implémenté. La donnée carbon-fr sert à l'exploration et au pré-dimensionnement, **jamais** à la comptabilisation réglementaire.

| Méthode (annexe, partie A, pt 6) | Donnée carbon-fr la plus proche | Écarts (à énoncer tels quels) |
|---|---|---|
| **(a)** moyenne **annuelle** nationale (Table 5, une des 5 dernières années au choix) | Seuil dérivé `~64 gCO₂eq/kWh` du ruleset `low-carbon:2025-2359` (ESTIMATION) | Lecture implicitement de type (a), mais appliquée **en continu** (à chaque créneau) et dérivée du budget produit — pas la Table 5. |
| **(b)** moyenne **horaire** du mix de la bidding zone **prévue day-ahead par le GRT** (H-2 avant gate closure) | `GET /v1/intensity/forecast` (`climatology@1` ; `acv-ademe@2` prévoit les entrées mix + imports par canal) | Modèle statistique **carbon-fr** (climatologie de l'historique RTE observé), **pas** la prévision du GRT ; horizon 72 h, pas un produit day-ahead H-2 avant gate closure. La granularité géographique coïncide par construction (FR = 1 bidding zone) ; la provenance, non. |
| **(c)** heures pleine charge → binaire `0` / `183 gCO₂eq/MJ` (selon les heures où le prix marginal était fixé par renouvelable/nucléaire) | Pilier `renewable-share` (part instantanée) ; technologie marginale de `/v1/price` | Aucun comptage d'« heures pleine charge » implémenté ; parenté d'esprit uniquement. |
| **(d)** intensité **horaire de la technologie marginale** fixant le prix, **si publiée par le GRT** | Le champ `marginal_technology` du contexte de `GET /v1/price` (`estimated: true` immuable) | Heuristique d'**ordre de mérite sur le mix domestique observé** — pas une donnée d'appel par centrale ; ne modélise pas le couplage de marché européen (le prix-setter réel peut être à l'étranger) ; RTE ne publie pas cette donnée → la condition « si publiée par le GRT » n'est **pas** satisfaite. |

**Conversion (didactique uniquement).** `1 kWh = 3,6 MJ` ⇒ `gCO₂eq/MJ = gCO₂eq/kWh ÷ 3,6`. L'unité canonique de l'API reste **gCO₂eq/kWh** ; aucune valeur en gCO₂eq/MJ n'est exposée. ⚠️ La valeur France de la Table 5 (18,8 en 2019 / 18,3 en 2021 gCO₂eq/MJ, soit ≈ 66-68 gCO₂eq/kWh) est **numériquement proche** de notre proxy `~64` : coïncidence d'ordre de grandeur (le nucléaire domine les deux dérivations), **pas** une équivalence méthodologique — les deux valeurs ne doivent jamais être présentées comme se validant l'une l'autre.

**Répercussions.** (1) Une phrase + lien dans le README (paragraphe `?eligibility=`) ; (2) doc-comments OpenAPI enrichis (`/v1/intensity/forecast` et `/v1/intensity/greenest-window` : « pas la prévision day-ahead du GRT » ; technologie marginale de `/v1/price` : renvoi méthode (d)) ; (3) le `legal_basis` du ruleset `low-carbon:2025-2359` mentionne l'existence des quatre méthodes et le fait que le seuil `~64` est un proxy hors annexe. Ce dernier point modifie un **texte servi** d'un ruleset : la passe de neutralité adversariale a été re-jouée sur le wording ([revue, §7](0026-revue-neutralite.md)).
