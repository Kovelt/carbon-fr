...
> 1. **Propriétaire crates.io** — **tranché le 2026-09-24 : `Kovelt`** (équipe de l'organisation GitHub ajoutée comme propriétaire après la première publication, qui se fait forcément depuis un compte personnel). Question d'origine : compte GitHub personnel (`Morgan-Voltz`) ou organisation `Kovelt` ? *Recommandation : `Kovelt`*, cohérent avec `repository = "https://github.com/Kovelt/carbon-fr"` et avec le Trusted Publisher npm déjà déclaré sous `Organization or user: Kovelt` pour le SDK TS (`release-sdk.yml:12`).
