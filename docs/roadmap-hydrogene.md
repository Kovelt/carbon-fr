...
| H2 | **GATE de neutralité** | Revue adversariale multi-agents de la sortie réelle des deux cadres (méthode ADR-0024 : critiques pro/anti-nucléaire + audits structurels), verdict daté dans `docs/adr/` | Avant tout palier payant sur cette couche (recommandé par ADR-0026) | ☐ |
...
| H4 | **`MixForecast`** | Part renouvelable prévisionnelle → verdicts `rfnbo` déterminés sur l'horizon futur (le domaine consomme déjà `Option<f64>`) ; ⚠️ distinct du `MixForecaster` GBDT d'ADR-0013, synergie à arbitrer | Valeur montera avec la corrélation horaire ; sensé après H3 | ☐ réserve |
...
| H6 | **Couche B-light** | Page carte « électrolyseurs FR/UE × intensité carbone live » (registre RTE data.gouv + Observatoire européen de l'hydrogène, ingestion trimestrielle, **sans API structurelle**) — le différenciateur selon ADR-0025 | Parc FR visible (SNH II : premiers GW ~2027-2028) ; peut accompagner le site statique o2switch | ☐ réserve |
